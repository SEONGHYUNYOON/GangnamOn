...
1. 미니홈피 프로필 사진 업로드 실패 버그 수정 (2차 수정 포함) —
   src/lib/imageUpload.js와 src/App.jsx에 다단계 fallback을 추가했어
   (리사이즈 실패 시 원본 파일 사용 → 권한 지정 업로드 실패 시 권한 없이
   재시도 → 프로필 문서 직접 업데이트 실패 시 economy Function의 새
   update_avatar 액션으로 재시도). 1차 배포 후에도 계속 실패한다는 리포트를
   받아서 다시 코드를 살펴봤고, 진짜 원인으로 보이는 버그를 하나 더
   찾아서 고쳤어: 이미지 리사이즈에 쓰던 canvas.toBlob('image/webp', ...)이
   일부 브라우저에서 webp 인코딩을 지원하지 않을 때 에러를 던지지 않고
   조용히 null을 반환하는데, 그 null이 그대로 4바이트짜리 깨진 파일로
   업로드 시도되면서 원인을 알 수 없는 실패로 이어지고 있었어. 이제
   webp가 실패하면 모든 브라우저가 지원하는 png로 자동 재시도하고, 그마저
   실패하면 원본 파일의 확장자를 미리 검사해서 (jpg/jpeg/png/webp가 아니면
   업로드를 시도하지 않고) 명확한 에러 메시지를 보여주도록 했어. 또한 실패
   알림 문구에 실제 에러 메시지를 괄호로 함께 보여주도록 해서, 혹시 또
   실패하더라도 사용자가 알림 문구만 캡처해서 보내주면 바로 원인을 알 수
   있게 만들었어.
...
4. 채팅창 UI 버그 2건 수정 (순수 프론트엔드 변경, 스키마/Function 변경
   없음) — src/components/ChatWidget.jsx:
   - 대화가 길어지면 메시지 입력창이 화면 밖으로 밀려서 사라지던 버그를
     고쳤어 (CSS grid 레이아웃에 min-h-0을 추가해서 메시지 목록 영역만
     스크롤되고 입력창은 항상 보이게 함).
   - 지금 그 대화방을 보고 있는 중이어도(브라우저를 켜놓고 접속 중일 때)
     상대가 메시지를 보내면 팝업 알림이 뜨도록 했어 (예전에는 다른 방에
     메시지가 왔을 때만 팝업이 떴었음).
...
```
git add -A
git commit -m "관리자 메뉴(신규회원/전체공지) + 프로필 사진 업로드 버그 수정 + 푸시알림 economy 통합 + 채팅 UI 버그 수정"
git push
```
...
- 프로필 사진 업로드가 실패 없이 정상적으로 되는지 (특히 예전에 실패했던
  계정으로 재확인)
- 계정 두 개로 채팅 대화를 길게 이어가도 입력창이 계속 화면에 보이는지
- 대화방을 열어놓은 상태에서 상대가 메시지를 보냈을 때도 팝업 알림이
  뜨는지
- 문제 없으면 결과 요약해서 알려줘.
```
