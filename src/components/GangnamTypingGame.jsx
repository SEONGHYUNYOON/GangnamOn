...
const GangnamTypingGame = ({ onClose, user }) => {
     ...
     return (
          <div className="fixed inset-0 z-[70] bg-[#0A0A10] flex items-center justify-center p-4">
               ...
               <div className={`relative bg-gray-900/80 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-pink-500/30 shadow-[0_0_80px_rgba(236,72,153,0.2)] max-w-6xl w-full flex flex-col lg:flex-row gap-8 items-center lg:items-start animate-in zoom-in-95 duration-500 ${shake ? 'animate-shake' : ''}`}>
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col gap-6">
                         ...
                         <div className="flex flex-row lg:flex-col gap-4 w-full">
                              <div className="flex-1 bg-black/40 p-5 rounded-2xl border border-pink-500/30 shadow-[inset_0_0_15px_rgba(236,72,153,0.1)]">
                                   ...
                                   <div className="text-4xl font-black text-white font-mono bg-clip-text text-transparent bg-gradient-to-r from-pink-300 to-purple-300">
                    ...
                    <div className="w-full lg:w-2/3 flex flex-col items-center relative">
                         <div className="relative rounded-t-3xl overflow-hidden border-2 border-b-0 border-pink-500/50 bg-black/80 w-full shadow-[0_-10px_40px_rgba(236,72,153,0.15)]" style={{ height: GAME_H }}>
                              ...
                              {/* Words */}
                              {words.map(w => (
                                   <div key={w.id} className="absolute text-xl font-black text-white px-3 py-1 rounded-lg bg-pink-600/20 border border-pink-400/50 backdrop-blur-sm shadow-[0_0_15px_rgba(236,72,153,0.4)] whitespace-nowrap" style={{ left: w.x, top: w.y }}>
                                        {w.word}
                                   </div>
                              ))}
                         ...
                         {/* Input Area */}
                         <div className="w-full bg-gray-900 border-2 border-pink-500/50 rounded-b-3xl p-4 flex gap-4 shadow-[0_10px_40px_rgba(236,72,153,0.15)] relative z-10">
                              <input
                                   ref={inputRef}
                                   type="text"
                                   value={input}
                                   onChange={e => {
                                        ...
                                        if (e.target.value.length > input.length) soundManager.playTick(); // subtle typing sound
                                   }}
                                   disabled={!gameStarted || gameOver}
                                   placeholder={gameStarted && !gameOver ? "여기에 단어를 입력하세요..." : "게임을 시작해주세요"}
                                   className="w-full bg-black/50 border border-pink-500/30 rounded-xl px-6 py-4 text-2xl font-bold text-white placeholder-pink-900/50 focus:outline-none focus:border-pink-400 focus:shadow-[0_0_20px_rgba(236,72,153,0.4)] transition-all text-center"
                              />
                         </div>
                    </div>
               </div>
